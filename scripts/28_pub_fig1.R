...
if (stage %in% c("legend", "panels", "all")) {
  ...
  l <- c(
    ...
    sprintf("- %d genes reach FDR < 0.05 for T2D vs control and only %d of those also clear |log2FC| > %.1f; carrying that set through the strict downstream rules leaves one intersecting gene (SREBF2, decision R17). **b** therefore uses nominal P < %.2f and the DEG set is exploratory (decision M9).",
            deg[adj.P.Val < 0.05, .N], deg[adj.P.Val < 0.05 & abs(logFC) > LFC, .N], LFC, PCUT),
